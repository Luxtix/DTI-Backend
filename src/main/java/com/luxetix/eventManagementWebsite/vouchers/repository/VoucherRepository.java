...
public interface VoucherRepository extends JpaRepository<Vouchers,Long> {
    public static final String eventVoucherQuery = "select v.id as voucherId, v.name as voucherName, v.rate as voucherRate , v.startDate as startDate , v.endDate as endDate, v.voucherLimit as voucherLimit from Vouchers v left join Events e on v.events.id = e.id  WHERE e.id = :eventId\n" +
            "  AND (\n" +
