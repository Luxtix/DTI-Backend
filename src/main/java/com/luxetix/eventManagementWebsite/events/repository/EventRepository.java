...
public interface EventRepository extends JpaRepository<Events,Long> {
    ...
    public static final String eventDetailsQuery = "SELECT e.id AS eventId, e.name AS eventName, e.cities.name AS cityName, e.categories.name as categoryName, e.address AS address, e.eventImage AS eventImage, e.venueName AS venueName, e.descriptions AS description, e.eventDate AS eventDate, e.startTime AS startTime, e.endTime AS endTime, e.isOnline AS isOnline, e.isPaid AS isPaid, u.fullname AS organizerName, u.avatar AS organizerAvatar, (SELECT COUNT(fe.id) FROM FavoriteEvents fe WHERE fe.events.id = e.id) AS favoriteCounts, (CASE WHEN EXISTS (SELECT 1 FROM FavoriteEvents fe2 WHERE fe2.events.id = e.id AND fe2.users.id = :userId) THEN TRUE ELSE FALSE END) AS isFavorite FROM Events e JOIN e.users u  WHERE e.id = :eventId GROUP BY e.id, u.id,e.categories.name, e.cities.name";
