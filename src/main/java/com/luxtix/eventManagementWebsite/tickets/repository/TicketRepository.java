...
public interface TicketRepository extends JpaRepository<Tickets,Long> {
    ...
    public static final String ticketSaleQuery = "SELECT ds.date_start AS date, COALESCE(SUM(tr.total_qty), 0) AS total_qty FROM (SELECT DISTINCT DATE_TRUNC(:intervalStart, generate_series(:start_date, :end_date, :intervalTime)) AS date_start) ds LEFT JOIN transactions tr ON DATE_TRUNC(:intervalTo, tr.created_at) = ds.date_start LEFT JOIN events e ON e.id = tr.event_id WHERE (e.id = :eventId OR e.id IS NULL) GROUP BY ds.date_start ORDER BY ds.date_start";
